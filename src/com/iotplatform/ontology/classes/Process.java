...
public class Process extends Class {
	...
	public Process(String name, String uri, Prefix prefix, String uniqueIdentifierPropertyName,
	...
	public Process() {
		super("Process", "http://purl.oclc.org/NET/ssnx/ssn#Process", Prefix.SSN, null, true);
		init();
	}
	...
	private void init() {
		...
		this.getProperties().put("hasInput", new ObjectProperty(getProcessSubjectClassInstance(), "hasInput",
				Prefix.SSN, Input.getInputInstance(), true, false));

		this.getHtblPropUriName().put(Prefix.SSN.getUri() + "hasInput", "hasInput");

		this.getProperties().put("hasOutput", new ObjectProperty(getProcessSubjectClassInstance(), "hasOutput",
				Prefix.SSN, Output.getOutputInstance(), true, false));

		this.getHtblPropUriName().put(Prefix.SSN.getUri() + "hasOutput", "hasOutput");
	...
	public static void initProcessStaticInstance(Process processInstance) {
		...
		processInstance.getProperties().put("hasInput",
				new ObjectProperty(processInstance.getProcessSubjectClassInstance(), "hasInput", Prefix.SSN,
						Input.getInputInstance(), true, false));

		processInstance.getHtblPropUriName().put(Prefix.SSN.getUri() + "hasInput", "hasInput");

		processInstance.getProperties().put("hasOutput",
				new ObjectProperty(processInstance.getProcessSubjectClassInstance(), "hasOutput", Prefix.SSN,
						Output.getOutputInstance(), true, false));

		processInstance.getHtblPropUriName().put(Prefix.SSN.getUri() + "hasOutput", "hasOutput");
